...
In order to produce a nowcast for a specific reference date, we use the number of already observed cases, $x_{t,d}$ and the delay PMF, $\pi$ to estimate the expected final count of cases $X_{t} = \sum_{d=1}^D X_{t,d}$. For notational simplicity, we will refer to $X_{t}$ as $N$, to indicate the random variable for the total number of final cases at a particular reference time $t$, and $X_{t,d}$ as $X$ to indicate the random variable for the number of cases at a particular reference time $t$ and delay $d$.

We will denote the CDF of the delay distribution at a particular delay $d$, $\sum_{d=0}^d \pi(d)$ simply by $\pi$.
...
$$
\text{pr}(N = n | X = x) = \frac{\binom{n}{x} \pi^x(1-\pi)^{n-x}}{\sum_{i=1}^\infty \binom{1}{x}\pi^x(1-\pi)^{i-x}}
$$

Plugging into $\mathbb{E}[N | X = x] = \sum_{n=0}^{\infty}\text{P}(N = n | X = x) n$ we get the following (omitting terms for $n<x$, which are 0):

$$
\mathbb{E}[N | X = x] = \sum_{n=x}^{\infty}n \frac{\binom{n}{x} \pi^x(1-\pi)^{n-x}}{\sum_{i=x}^\infty \binom{i}{x}\pi^x(1-\pi)^{i-x}}
$$

Which is equivalent to:

$$
\mathbb{E}[N | X = x] =  \frac{\sum_{n=x}^{\infty}n \binom{n}{x} \pi^x(1-\pi)^{n-x}}{\sum_{i=x}^\infty \binom{i}{x}\pi^x(1-\pi)^{i-x}}
$$
...
$$
\mathbb{E}[N | X = x]  = \frac{(x + 1 -\pi)/\pi^2}{1/\pi} = \frac{x + 1 - \pi}{\pi}
