...
In the case where we have missing values in the bottom right (i.e. we have a reporting triangle), the estimator (\pi\_d) can only be evaluated after discarding all data from the last $D-1$ time points. In order to use the partial information contained in the most recent $D-1$ time points, we use a different representation of the delay distribution via terms of the form:
